...
import {calculateScore, ANSWERS_AMOUNT, MAX_LIVES_AMOUNT, AnswerType} from './calculate-score';
...
describe(`Calculate score`, () => {
	...
	it(`should calculate score correctly`, () => {
		...
		answers = new Array(ANSWERS_AMOUNT).fill({status: AnswerType.CORRECT, timeElapsed: 15});
		...
		answers = new Array(ANSWERS_AMOUNT).fill({status: AnswerType.FAST, timeElapsed: 5});
		...
		answers = new Array(ANSWERS_AMOUNT).fill({status: AnswerType.SLOW, timeElapsed: 25});
	...
	it(`should return -1 if answers amount less then ${ANSWERS_AMOUNT}`, () => {
		const answers = new Array(ANSWERS_AMOUNT - 1).fill({status: AnswerType.CORRECT, timeElapsed: 15});
	...
	it(`should return -1 if livesLeftAmount amount less then 0 or more then ${MAX_LIVES_AMOUNT}`, () => {
		const answers = new Array(ANSWERS_AMOUNT).fill({status: AnswerType.CORRECT, timeElapsed: 15});
